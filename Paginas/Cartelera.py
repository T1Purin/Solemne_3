...
def cartelera():
    ...
    # Cargar reseñas desde el archivo CSV al inicio
    def cargar_reseñas(archivo='Archivos/reseñas.csv'):
        try:
            ...
            if 'id' not in df.columns or 'name' not in df.columns or 'review' not in df.columns:
                ...
                return {}
            df = df.dropna(subset=['id', 'name', 'review'])  # Eliminar filas con valores nulos en columnas esenciales
    ...
    # Función para agregar una nueva reseña al archivo CSV
    ...
    if "selected_movie_genre" in st.session_state:
        ...
        # Crear un espacio para ingresar el nombre y la reseña
        name = st.text_input("Ingresa tu nombre:")
        review = st.text_area("Escribe tu reseña:")

        # Cargar reseñas desde el archivo CSV si aún no está en session_state
        ...
        if st.button("Enviar reseña"):
            if name and review:
                ...
                # Llamar a la función para agregar la reseña al archivo CSV
